...
int CreateTaskFile()
{
    NCShared::LogFileMessage("Creating tasks file");
    ...
    {
        NCShared::LogFileError("Failed to open task file for initialization!");
        return 0;
    }
    ...
    NCShared::LogFileMessage("JSON content: " + content);
    tasksFile << content;
    tasksFile.flush(); // Force write to disk
    NCShared::LogFileMessage("Initiazed task file");
...
int TaskDueToUnixTime(std::string taskDue)
{
    ...
    NCShared::LogFileMessage("amount: " + amountStr + " size: " + sizeStr);
    ...
    {
        NCShared::LogFileError("Invalid due input: not a number");
        return -1;
    }
    catch (const std::out_of_range& e)
    {
        NCShared::LogFileError("Due number out of range for int");
        return -1;
    }
